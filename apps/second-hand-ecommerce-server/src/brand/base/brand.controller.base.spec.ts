...
const service = {
  ...
  brands: () => FIND_MANY_RESULT,
  brand: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("Brand", () => {
  ...
  test("GET /brands/:id non existing", async () => {
    await request(app.getHttpServer())
  ...
  test("GET /brands/:id existing", async () => {
    await request(app.getHttpServer())
  ...
  test("POST /brands existing resource", async () => {
